...
class ClickhouseDialectTest
  ...
  test("throws exception for unsupported spark version") {
    val exception = intercept[UnsupportedOperationException] {
      ClickhouseDialectRegistry.dialectClassName("3.4.0")
    }
  ...
  val testReadArrayCasesV0_7_X = Table(
    ...
    (
      ...
      "([1.23, 2.34, 3.45, 4.56, 5.67])",
      ArrayType(DecimalType(9, 2), containsNull = false)),
    // On Spark 4 these signed numeric arrays read successfully even with driver 0.7.x,
    // unlike on Spark 3.5 where they raised a ClassCastException.
    (
      "shortArrayColumn Array(Int16)",
      "([1, 2, 3, 4, 5])",
      ArrayType(ShortType, containsNull = false)),
  ...
  forAll(
    ...
    (columnDefinition: String, insertedData: String, expectedType: DataType) =>
      test(s"read ClickHouse Array for ${columnDefinition} column") {
        ...
        // In Scala 2.13 `Row.getAs[Seq[_]]` returns a mutable ArraySeq, which is not an
        // immutable Seq; use the collection-root Seq to stay version-neutral.
        df.collect().map(_.getAs[scala.collection.Seq[Any]](df.schema.fields.head.name)).head
        assert(df.count() == 1)
      }
  }

  // On Spark 4 these still fail, but as a COLUMN_ARRAY_ELEMENT_TYPE_MISMATCH error instead of
  // the driver-level ClassCastException seen on Spark 3.5. The signed numeric types that used to
  // be here (Int16/Int32/Int64/Float32/Float64) now read successfully, see testReadArrayCasesV0_7_X.
  ...
  forAll(
    ...
    (
      ...
      test(s"cannot read ClickHouse Array for ${columnDefinition} column") {
        ...
        // but read is failing (on Spark 4 the driver-level ClassCastException surfaces as a
        // SparkSQLException with error class COLUMN_ARRAY_ELEMENT_TYPE_MISMATCH)
